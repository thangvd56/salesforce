...
class MyApp < Sinatra::Base
  ...
  get '/' do
    ...
    @accounts= client.query("select Id, Name, username__c, email__c, Employee_ID__c, Position__c from Employees__c")    
    erb :index
  end
  ...
  get '/details/:id' do
    @employees = client.query("select Name, Position__c, Hiredate__c, birthday__c, Address__c, email__c, Phone__c from Employees__c where Id = '#{params[:id]}'")
    erb :details
  end
